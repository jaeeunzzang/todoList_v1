var total = 0; //리스트 갯수. 아이디에 추가해줄 변수
	...
	function addNew() { //name, checked ->함수명: appendItem 체크가 된건지 안된건지만 중요,,...........
		var text = document.getElementById("addList").value; //addList에 입력된 값 가져오기

		if (text.length < 1) { //입력된 값이 없으면
			alert("할일을 입력하세요");
			return false;
		} else { //값이 입력되었으면
...
function createItem (taskTitle, id,checked) {
  ...
  return item;
	...
	function updateList() { //체크박스 누를때 줄그어주고 옮겨주고 하는 함수
